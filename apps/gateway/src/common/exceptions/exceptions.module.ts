 ...
 * - GatewayExceptionFilter (@Catch(GatewayException)) 最具体，会优先匹配 GatewayException
 * - HttpExceptionFilter (@Catch(HttpException)) 次具体，会匹配所有 HttpException（包括其子类）
...
@Module({
  providers: [
    ...
    {
      provide: APP_FILTER,
      useClass: GatewayExceptionFilter, // 最具体的（@Catch(GatewayException)），优先处理网关异常
    },
    {
      provide: APP_FILTER,
      useClass: HttpExceptionFilter, // 次具体的（@Catch(HttpException)），处理所有 HTTP 异常
    },
    {
      provide: APP_FILTER,
  ...
  exports: [AllExceptionsFilter, HttpExceptionFilter, GatewayExceptionFilter],
})
