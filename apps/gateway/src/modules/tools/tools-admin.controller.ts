...
export class ToolsAdminController {
  ...
  async list(@Req() req: Request, @Query('page') page?: string, @Query('pageSize') pageSize?: string, @Query('search') search?: string) {
    ...
    return await this.rpc('admin.tools.list', {
      actor,
  ...
  async findOne(@Req() req: Request, @Param('id') id: string) {
    const actor = actorFromRequest(req);
    return await this.rpc('admin.tools.findOne', { actor, id });
  }

  @Post()
  async create(@Req() req: Request, @Body() body: any) {
    const actor = actorFromRequest(req);
    return await this.rpc('admin.tools.create', { actor, ...body });
  }

  @Patch(':id')
  async update(@Req() req: Request, @Param('id') id: string, @Body() body: any) {
    const actor = actorFromRequest(req);
    return await this.rpc('admin.tools.update', { actor, id, ...body });
  }

  @Delete(':id')
  async remove(@Req() req: Request, @Param('id') id: string) {
    const actor = actorFromRequest(req);
    return await this.rpc('admin.tools.remove', { actor, id });
  }

  @Get(':id/versions')
  async versions(@Req() req: Request, @Param('id') id: string) {
    const actor = actorFromRequest(req);
    return await this.rpc('admin.tools.versions', { actor, id });
  }
}
